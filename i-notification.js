const iNotification = (props) => {

    const {type = 'success', title, mes, position = 'top-right', icon, time, progress = true} = props;
    ...
    const getNotification  = () => {
        ...
        _progress.className = "i-notification-progress";
        _progress.style.animationDuration = `${time / 1000}s`;

        notification.appendChild(head);
        notification.appendChild(body);
        notification.appendChild(close);
        if (progress) notification.appendChild(_progress);
        ...
        notification.addEventListener('mouseenter', () => {
            notification.classList.add('i-notification-hovered');
            clearInterval(timeout);
        })

        notification.addEventListener('mouseleave', () => {
            notification.classList.remove('i-notification-hovered');
            timeout = setTimeout(() => {
                hide();
            }, time ? time + 100 : defaultTime + 100);
        })
